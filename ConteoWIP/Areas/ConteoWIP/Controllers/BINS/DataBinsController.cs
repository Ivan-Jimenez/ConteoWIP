        ...
        private void ExportDataToExcel(string area, string count_type)
        {
            ...
            using (SqlConnection sqlConn = new SqlConnection(conn))
            {
                ...
                if (count_type.Equals("Count"))
                    query = String.Format("SELECT Product, OrderNumber, OperationDescription, OrdQty FROM CountBINS WHERE AreaLine='{0}' AND Physical1 != OrdQty OR ISNULL(Physical1, 0) = 0;", area);
                else if (count_type.Equals("All"))
                    query = "SELECT * FROM CountBINS WHERE Status != 'OK' OR ISNULL(Status, 'Fuck')='Fuck';;";
                else
                    query = String.Format("SELECT Product, OrderNumber, OperationDescription, OrdQty FROM CountBINS WHERE AreaLine='{0}' AND ReCount != OrdQty OR ISNULL(ReCount, 0) = 0;", area);
